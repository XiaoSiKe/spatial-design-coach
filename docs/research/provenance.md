...
2026-09-03 按维护者提供的 50 本清单选择 24 本专业著作，并增加《庄子》选篇；逐项来源、版本、原文选读 E／导读 M、访问缺口以[来源地图](./source-map.md#selected-readings)为唯一记录，不在此另建重复书目。

运行时保存本项目原创的教学提问、空间动作、反例和验证方法。未复制现代书籍正文、插图、出版社样章、现代注释或译文，也未将它们改用 MIT 许可再发布。MoMA 等公开试读只用于查核论点，不随仓库分发。古籍原典与现代校点、注释、翻译分别对待；《庄子》只作选篇理解及明确标注的教学转译，不复制网页附加说明。

M 条目保留原著正文核读缺口。模型熟悉书名、生成方法卡或通过行为测试，均不能证明已经研读全书；新增精确引文前必须取得并检查实际版本。
