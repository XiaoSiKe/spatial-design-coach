...
完整报告需要接受少量失败时，可在上述命令追加 `--max-failed 2`（例如 91/93）。这不改变原始判定、`all_passed` 或严格的 `release_ready`；完整评测存在失败时仍返回非零退出码。发布检查另行核对失败数与显式额度，保留关键复跑和禁止行为门槛，默认额度仍为零。

本次 0.6.1 维护者已接受此前 91/93 的结果，并明确要求停止追加评测、直接交付。Release 因而记录采用的历史证据、后续修正的实际验证及被中止队列的完成范围；这是明确的维护者验收，不宣称当前提交的完整发布资格脚本通过。旧提交的完整报告不能改称当前提交的完整结果，未跑完的队列不能标成全部通过。
