...
2. 评判者取得原始请求、项目身份元数据和必要 fixture 上下文，根据可观察输出检查 `must` 和 `must_not`；不能凭缺失的输入猜测事实。正式名称本身不等于成绩或正确性承诺，仍须单独检查实际能力声明。
...
判据须与当轮目标和可用材料相称：不把后续制作阶段才需要的参数，提前变成纯叙事建议必须重复声明的缺口。必要检查仍须提出，任何已知事实、数值规格和完成状态均不得虚构。
...
评判书目版本、ISBN 等信息时，检查随 Skill 提供的 `design-lenses.md` 来源记录；这些记录能支持书目事实，不能证明学生所用版本、原文章节或场地性能。评测包装器不另加固定词数上限，沿用 Skill 的简洁且完整回复合同。精确图面位置须对齐 SVG 与 PNG 的坐标基准，不能直接比较不同缩放下的像素值。剖线验收关注可定位的锚点、主要过程／使用冲突和有依据的空间关系；不额外要求穷举全部途经图形或与当前检验无关的完整河槽。明确断言的错误位置、虚构路径和未定位地物仍不通过。
...
本次 0.6.1 维护者明确接受至少 **91/93**，可在上述发布命令追加 `--max-failed 2`。这是本次发布的显式验收额度，不改变原始判定、`all_passed` 或严格的 `release_ready`；完整评测存在失败时仍返回非零退出码。发布检查另行核对失败数与该额度，保留关键复跑和禁止行为门槛。Release 必须公开实际通过数、失败情境和接受的数量，不能写成全部通过。后续发布默认额度仍为零。
